...
async def generate_plan(body: PlanGenerateRequest, current_user: CurrentUser, db: SessionDep):
    ...
    simulation_id = db.execute(select(Simulation.id).where(Simulation.user_id == current_user.id, Simulation.target_job == body.target_job)).scalar_one_or_none()
    ...
    for p_data in phases_data:
        phase = Phase(
            name=p_data.get("name", "Unnamed Phase"),
            duration_weeks=p_data.get("duration_weeks", 1),
            hours=p_data.get("hours", 0),
            topics=p_data.get("topics", []),
        ...
        for t_data in tasks_data:
            raw_deadline = t_data.get("deadline")
            deadline_val = None
            if raw_deadline and raw_deadline != "":
                try:
                    deadline_val = datetime.fromisoformat(str(raw_deadline).replace("Z", "+00:00"))
                except (ValueError, TypeError):
                    deadline_val = None

            task = Task(
                title=t_data.get("name", t_data.get("title")),
